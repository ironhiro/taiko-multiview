...
description: "태고 멀티뷰 개발 하네스의 오케스트레이터. 기능 수정·UI 개선·버그 수정 요청을 구현(multiview-dev) → 기기별 검증(device-qa) + 성능 전후 비교(perf-analyst) → 실패 시 재수정 루프로 처리하고 결과를 보고한다. '모바일 UI 개선', '상단 바 고쳐줘', '타일/레이아웃 수정', '버그 수정하고 검증까지', '성능 회귀 없는지 확인하면서 바꿔줘', '하네스로 해줘' 요청 시 반드시 사용. 후속 작업: 다시 실행, 재검증, QA만 다시, 성능만 다시 측정, 이전 결과 기반으로 수정·보완·업데이트, 실패 항목만 다시 고쳐줘 요청에도 이 스킬을 사용. 단순 질문(비용, 사용법)이나 배포만 하는 요청은 직접 답하거나 deploy-dev 스킬을 쓴다."
---
...
- **수용 기준**: 측정 가능한 문장으로(예: "iPhone SE에서 상단 바 높이 ≤ 120px")
- 성능 영향 여부: 타일·플레이어·스크롤·레이아웃 코드를 건드리면 "있음"
...
**정상 흐름.** "매장이 많으면 폰 상단 바가 두 줄이 돼" → Phase 1 수용 기준: 4개 기기에서 상단 바 높이 ≤ 120px, 가로 넘침 0 → 성능 영향 없음, Phase 2 생략 → dev가 넘치는 매장을 목록 하나로 접음 → QA가 shoot.mjs로 측정, e2e 통과 → PASS → 보고(실제 iPhone Chrome에서 볼 항목 명시).
