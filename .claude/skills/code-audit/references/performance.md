...
- 만들고 부수기(churn): 스크롤·가시성 변화마다 플레이어를 새로 만드는 경로가 있나. 예산·슬롯 규칙을 우회하는 경로(채팅 전환, 매장 전환, 스트림 id 변경).
...
## React 렌더링
- `App.tsx`의 상태 변경이 전체 그리드를 다시 그리나. `streamsByStation` 같은 파생 값의 메모이제이션, 콜백 안정성(`onRequestChat={() => ...}` 인라인 함수가 매 렌더 새로 생김 → 자식 효과 재실행 여부).
