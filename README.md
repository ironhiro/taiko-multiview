...
- 채팅: 타일의 채팅 버튼으로 그 방송의 라이브 채팅을 오른쪽에 표시
...
- 앱: 채팅 패널에서 바로 입력 가능. 패널 자리에 iframe 대신 별도 웹뷰를 겹쳐서 유튜브 로그인 유지 (iframe 안에서는 WebKit 추적 방지로 로그인 쿠키를 못 받음)
- 브라우저: 패널은 읽기 전용. 입력은 `유튜브에서 채팅 ↗` 으로 여는 유튜브 채팅 창(모바일은 새 탭)에서, 로그인이 필요하면 로그인을 거쳐 열림. 브라우저는 다른 사이트에 넣은 유튜브 채팅에 로그인을 넘기지 않음 (iPhone 의 모든 브라우저, 크롬 시크릿 탭)
- 채팅 iframe 은 sandbox 처리해 채팅 안의 로그인 링크가 멀티뷰 페이지를 떠나게 하지 못하고, 그 링크가 있는 하단 입력 영역은 잘라서 숨김
- 창 이탈 방지: 유튜브 플레이어가 창 전체를 로그인 페이지로 보내는 경우가 있어, 멀티뷰 창이 다른 사이트로 넘어가면 즉시 되돌림. 로그인 요청은 채팅 창으로, 나머지 링크는 기본 브라우저로
